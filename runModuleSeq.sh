...
function usage {
  ...
  will produce run-018449x001.shoe.root and run-018449x001.qa.root\n
...
FILEEXPORTEROUTPUT=shineOffline.root
QAOUTPUT=QAHistograms.root
...
### Save outputs under appropriate name.
mv $FILEEXPORTEROUTPUT ${OUTPUTFILEPREFIX}.shoe.root
ls ${OUTPUTFILEPREFIX}.shoe.root
if [[ -f $QAOUTPUT ]] ; then mv $QAOUTPUT ${OUTPUTFILEPREFIX}.qa.root; fi
